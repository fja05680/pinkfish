...
def profit_factor(tlog):
    if gross_profit(tlog) == 0:
        return 0
    if gross_loss(tlog) == 0:
        return 1000
...
def ratio_avg_profit_win_loss(tlog):
    if avg_profit_per_winning_trade(tlog) == 0:
        return 0
    if avg_loss_per_losing_trade(tlog) == 0:
        return 1000
    return avg_profit_per_winning_trade(tlog) / avg_loss_per_losing_trade(tlog) * -1
...
def stats(ts, tlog, dbal, start, end, capital):
    ...
    stats["drawdown_recovery"] = (
        _difference_in_years(
...
def summary3(stats, benchmark_stats, *extras):
    ...
    columns = ["strategy", "benchmark"]
    data = [
        (stats["annual_return_rate"], benchmark_stats["annual_return_rate"]),
        (stats["max_closed_out_drawdown"], benchmark_stats["max_closed_out_drawdown"]),
        (
            stats["drawdown_annualized_return"],
            benchmark_stats["drawdown_annualized_return"],
        ),
        (stats["pct_profitable_months"], benchmark_stats["pct_profitable_months"]),
        (stats["best_month"], benchmark_stats["best_month"]),
        (stats["worst_month"], benchmark_stats["worst_month"]),
        (stats["sharpe_ratio"], benchmark_stats["sharpe_ratio"]),
        (stats["sortino_ratio"], benchmark_stats["sortino_ratio"]),
    ]
    # add extra metrics
    for extra in extras:
        index.append(extra)
